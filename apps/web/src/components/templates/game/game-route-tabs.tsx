...
type GameRouteTabId = "overview" | "events" | "forums" | "players" | "matches";
...
export function GameRouteTabs({ game }: GameRouteTabsProps) {
  const t = useTranslations("GamePage");
  const pathname = normalizePath(usePathname());
  const basePath = `/games/${game.slug}`;

  const tabs: GameRouteTab[] = [
    {
      id: "overview",
    ...
    },
    {
      ...
      icon: CalendarDays,
      count: game._count?.events ?? 0,
    },
    {
      id: "forums",
      label: t("sectionTabForums"),
